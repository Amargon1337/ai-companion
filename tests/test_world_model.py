...
def test_entity_creation_and_matching(memory_store) -> None:
    store = memory_store
    f1 = Fact(
        id="f-world-1",
...
def test_relationship_layer(memory_store) -> None:
    store = memory_store
    f = Fact(
        id="f-world-rel-1",
...
def test_cognitive_queries_neglected_entities(memory_store) -> None:
    store = memory_store
    now_dt = datetime.now()
...
def test_get_entity_graph(memory_store) -> None:
    store = memory_store
    f = Fact(
        id="f-world-graph-1",
        fact="У Ивана есть собака Морзик",
