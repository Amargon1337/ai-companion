...
def record_ping_sent(reason: str, baseline_state: str, urgency: int, message: str) -> str:
    ...
    with _db()._conn() as conn:
        conn.execute(
            ...
            VALUES (?,?,?,?,?,?,1,0)""",
            (event_id, datetime.now().isoformat(), reason, baseline_state, urgency, message),
        )
    return event_id


def record_ping_reply(reply_delay_hours: float) -> None:
    with _db()._conn() as conn:
        row = conn.execute(
        ...
        if row:
            conn.execute(
                "UPDATE proactive_events SET user_replied=1,reply_delay_hours=? WHERE id=?",
